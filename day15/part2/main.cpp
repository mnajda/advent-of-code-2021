...
std::vector<std::vector<std::int64_t>> expand_input(std::vector<std::vector<std::int64_t>> input)
{
    auto expanded = std::vector<std::vector<std::int64_t>>(
        input.size() * 5, std::vector<std::int64_t>(input.front().size() * 5, 0));

    for (auto y = 0U; y < input.size(); ++y)
    {
        for (auto x = 0U; x < input.size(); ++x)
        {
            expanded[y][x] = input[y][x];
        }
    }

	for (auto y = input.size(); y < input.size() * 5; ++y)
    {
        for (auto x = 0U; x < input.front().size(); ++x)
        {
		    expanded[y][x] = (expanded[y - input.size()][x] == 9) ? 1 : expanded[y - input.size()][x] + 1;
        }
    }

	for (auto y = 0U; y < input.size() * 5; ++y)
    {
		for (auto x = input.front().size(); x < input.front().size() * 5; ++x)
        {
			expanded[y][x] =
                (expanded[y][x - input.front().size()] == 9) ? 1 : expanded[y][x - input.front().size()] + 1;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    auto input = expand_input(load_file(argv[1]));
    const auto result = find_shortest_path(input);
