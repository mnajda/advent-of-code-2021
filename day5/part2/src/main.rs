...
fn solve(input: Vec<(Coords, Coords)>) -> usize {
    ...
    for line in input {
        ...
        if begin.x == end.x {
            ...
            let xs: Box<dyn Iterator<Item = _>> = if begin.x < end.x {
                Box::new(begin.x..end.x + 1)
            } else {
                Box::new((end.x..begin.x + 1).rev())
            };
            let ys: Box<dyn Iterator<Item = _>> = if begin.y < end.y {
                Box::new(begin.y..end.y + 1)
            } else {
                Box::new((end.y..begin.y + 1).rev())
            };

            for (x, y) in xs.zip(ys) {
                *overlapping.entry(Coords { x, y }).or_insert(0) += 1;
            }
        }
    }
